...
function initializeApp(tgUser) {
    ...
    userRef.onSnapshot(async (doc) => {
        if (!doc.exists) {
            ...
            // ✅ Properly parse start_param and remove "ref_"
            ...
            console.log(`DEBUG: Parsed referrerId: ${referrerId}`);
                    ...
                    await db.runTransaction(async (transaction) => {
                        ...
                        if (refDoc.exists) {
                            transaction.update(referrerRef, {
                                ...
                                // Uncomment if you want to give referrer a signup bonus:
                                // balance: firebase.firestore.FieldValue.increment(SIGNUP_REF_BONUS),
                                // referralEarnings: firebase.firestore.FieldValue.increment(SIGNUP_REF_BONUS)
                            });
                        }
...
function updateUI() {
    ...
    const referralLink = `https://t.me/${TELEGRAM_BOT_USERNAME}?start=ref_${telegramUserId}`;
